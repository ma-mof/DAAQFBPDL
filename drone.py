...
def connect_drone(connection_string,baud, waitready=True):
    global vehicle
    if vehicle == None:
        vehicle = connect(connection_string,baud, wait_ready=waitready)
...
def send_movement_command_YAW(heading):
    ...
    msg = vehicle.message_factory.command_long_encode(
        ...
        speed,      #speed deg/s
        direction,  
        1,          #relative offset 1; absolute =0
        0, 0, 0)    
...
# detaljni opis naredbe položaja i brzine - NE KORISTI SE
def send_movement_command_XYA(velocity_x, velocity_y, altitude):
    ...
    msg = vehicle.message_factory.set_position_target_local_ned_encode(
        ...
        0b001110000111, #ignore velocity z and other pos arguments
        0, 0, altitude,
...
def send_movement_command_XYAH(velocity_x, velocity_y, altitude,heading):
    global vehicle

    speed = 0 #original =0 
      

    print("Sending XYZH movement command with v_x(forward/backward): %f v_y(right/left): %f yaw: %f" % (velocity_x,velocity_y,heading))

    msg = vehicle.message_factory.set_position_target_local_ned_encode(
        0,      
        0, 0,    
        mavutil.mavlink.MAV_FRAME_BODY_NED,  #relative to drone heading pos relative to EKF origin
        0b001110000111, #ignore velocity z and other pos arguments; 0b...--> bit1:PosX, bit2:PosY, bit3:PosZ, bit4:VelX, bit5:VelY, bit6:VelZ, bit7:AccX, bit8:AccY, bit9:AccZ, bit11:yaw, bit12:yaw rate
        0, 0, altitude,
        velocity_x, velocity_y, 0, 
        0, 0, 0, 
        heading, speed)    
...
#vehicle_1 = connect_drone('udp:127.0.0.1:14550')
#land()
